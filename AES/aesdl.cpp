...
void MixColumns2(byte mtx[4*4]){
	...
	for(int i=0; i<4; ++i)
	{
		...
		cout<<"tmp[0]:   "<<arr[0]<<endl;
		cout<<GFMul(0x03, arr[0]) <<endl;


		mtx[i] = GFMul(0x02, arr[0]) ^ GFMul(0x02, arr[1]) ^ arr[2] ^ arr[3];
		mtx[i+4] = arr[0] ^ GFMul(0x02, arr[1]) ^ GFMul(0x01, arr[2]) ^ arr[3];
		mtx[i+8] = arr[0] ^ arr[1] ^ GFMul(0x01, arr[2]) ^ GFMul(0x01, arr[3]);
		mtx[i+12] = GFMul(0x01, arr[0]) ^ arr[1] ^ arr[2] ^ GFMul(0x01, arr[3]);
	}
}
...
void InvMixColumns2(byte mtx[4*4])
{
	byte arr[4];
	for(int i=0; i<4; ++i)
	{
		...
		cout<<"tmp[0]:   "<<arr[0]<<endl;
		cout<<GFMul(0x0e, arr[0]) <<endl;


		mtx[i] = GFMul(0x0e, arr[0]) ^ GFMul(0x0e, arr[1]) ^ GFMul(0x0e, arr[2]) ^ GFMul(0x09, arr[3]);
...
int main()
{
	...
	// 加密，输出密文
	encrypt(plain, w);
// since plain is passed by reference, so plain is currently cipher
